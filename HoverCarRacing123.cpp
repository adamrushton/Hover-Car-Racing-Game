...
Huge to-do list
Burning fires on top of barrels - particle system for flames
Implement sound
Player car shooting stuff

Finishing touch
Remove magic numbers when finished
600 word report (write grade)
Scale map of the race course. Label all obstaces
*/
...
enum menuNames
{
	...
	PauseKey, QuitKey, ResetCameraKey, TopViewKey, prepareRaceKey, hoverCarForwardsKey, hoverCarBackwardsKey, hoverCarLeftKey,
...
public:
	string leftSettingsList[6] = { "Full Screen:", "FPS Toggle:", "Pause:", "Back/Quit:", "Reset Camera:", "Tilt up:" };
	...
	EKeyCode chaseCamera = Key_1;			// None changable key
	...
	void ResetKeys()
	{
		...
		viewTiltUp = Key_8;				   // Look up camera 
		...
		hoverCarBoost = Key_Return;	       // Boost
		...
		currentGameKeysLeft[TiltUpText] = "8";           // Default camera view 1 key
		...
		currentGameKeysRight[BoostText] = "Return";      // Default camera view 2 key
	}
	...
	void LoadSettings()
	{
		...
		loadData >> loadFullScreen >> toggleFpsCounter >> currentPauseKey >> currentQuitKey
			>> currentResetCamera >> currentViewTiltUp >> currentprepareRace >> currenthoverCarForwards
...
public:
	...
	{
		// Create
		if (bombNumber < 4)      bomb = bombMesh->CreateModel(0.0f, bombFloor, 100.0f + (100.0f * bombNumber));
		else if (bombNumber < 9) bomb = bombMesh->CreateModel(400.0f, bombFloor, 100.0f * (bombNumber - 4));
		// Assign box
		SetBox();
	}
...
int winnerTime;
IFont* countdownFont;
Countdown* threeSecCD = new Countdown(0005);
...
private:
	...
	Countdown* bounceTimer = new Countdown(3000);
	Countdown* displayTimer = new Countdown(5000);
	Countdown* assignSpeedTimer = new Countdown(3000);
	Countdown* shakeTimer = new Countdown(2000);
	Countdown* healthTimer = new Countdown(3000);
	Countdown* slowTimer = new Countdown(5000);
	Countdown* poisonTimer = new Countdown(5000);
	string userName;
	...
	float speedLimit = 0.6f;			   // 0.3 at uni, 0.6 at home
...
public:
	...
	{   // 0-1, 2-3, 4-5
		if (carNumber == 0) hoverCar = hoverCarMesh->CreateModel();
		...
		if (carNumber == 1) hoverCar->SetSkin("spred.jpg");
		if (carNumber == 2) hoverCar->SetSkin("spyellow.jpg");
		if (carNumber == 3) hoverCar->SetSkin("spdarkblue.jpg");
		if (carNumber == 4) hoverCar->SetSkin("sppink.jpg");
		if (carNumber == 5) hoverCar->SetSkin("sporange.jpg");
	}
	...
	{
		(cameraShake) ? myCamera->MoveY(0.3f) : myCamera->MoveY(-0.3f);
		cameraShake ^= 1;
	}
	...
	void SlowPackage(float minX, float maxX, float minZ, float maxZ, int crate, IFont* gameFont)
	{
		...
		if (slowTimer->CountingDown())
		{
			...
			slowness << "ENGINE FAILURE..." << slowTimer->GetTimeRemaining();
	...
	void PoisonPackage(float minX, float maxX, float minZ, float maxZ, int crate, IFont* gameFont, ICamera* myCamera)
	{
		...
		if (poisonTimer->CountingDown())
		{
			poison << "POISON... -" << ranDamage << " " << poisonTimer->GetMSRemaining() << "...";
			if (poisonTimer->GetMSRemaining() < 4000 && poisonTimer->GetMSRemaining() > 3950 ||
				poisonTimer->GetMSRemaining() < 3000 && poisonTimer->GetMSRemaining() > 2950 ||
				poisonTimer->GetMSRemaining() < 2000 && poisonTimer->GetMSRemaining() > 1950 ||
				poisonTimer->GetMSRemaining() < 1000 && poisonTimer->GetMSRemaining() > 950)
			{
				GenDMG();
				hitpoints -= ranDamage;
				ShakeCamera(myCamera);
	...
	void Bounce()
	{
		if (bounceTimer->GetTimeRemaining() == 3)
		{
			if (currentSpeed > 0.0f)      currentSpeed = -currentSpeed / 2.0f;
			else						  currentSpeed -= currentSpeed * 2.0f;
...
public:
	...
	void Update()
	{
		...
		if (distance < range && shoot == -1) shoot = 500;
...
public:
	...
	void CarThrough(float carX, float carZ, int currentCP, int car, IFont* gameFont)
	{
		if ((carX >= goThroughMinX && carX <= goThroughMaxX && carZ >= goThroughMinZ && carZ <= goThroughMaxZ) &&
			hoverCars[car]->GetGameState() == 4 && currentCP == 0 && hoverCars[car]->GetLap() < 2)
		{
			hoverCars[car]->NextLap();       // Increase lap
			hoverCars[car]->SetGameState(0); // Reset game state
		}
		if (currentCP == 0 && (hoverCars[playerCar]->GetLap() == 2))
		{	// Game over
...
int ButtonHandler(I3DEngine* myEngine, int gameState, int mouseX, int mouseY, IFont* gameFont)
{
	...
	if (myEngine->KeyHit(userSettings.buttonClicker)) // When you click the mouse button
	{
		...
		if (gameState == Settings) // When in the settings
		{
			...
			{												  // View Tilt Up Key
				userSettings.keyChangeName = userSettings.leftSettingsList[TiltUpText]; // Changing the top view key
				return BindNewKey;
			}
	...
	if (!gamePaused)
	{
		...
		{   // Check for collision between car and sides of the check point
			...
			{   // Check for the car going through the checkpoint
				...
				if (i == 0 && (hoverCars[car]->GetLap() == 2)) hoverCars[car]->Stop(car);
		...
		{	// Check collision between player car and all cars
			if (car > 0)
			{
				hoverCars[playerCar]->DummyCollisions(hoverCars[car]->GetMinX() - carWidth, hoverCars[car]->GetMaxX() + carWidth,
		...
		if (myEngine->KeyHit(userSettings.prepareRace))
		{
			hoverCars[playerCar]->PrepareRace();
		...
		{
			myCamera->ResetOrientation();
			myCamera->SetPosition(0.0f, 20.0f, -50.0f);
		}

		if (myEngine->KeyHeld(userSettings.viewTiltUp))
		{
			myCamera->ResetOrientation();
		}
